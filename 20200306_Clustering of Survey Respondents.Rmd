...
feature_info<-data %>%
    ...
    colnames(feature_info)<-data$uid

# Create a dataframe containing information about the respondent
family_info<-data %>%
    select(uid, ed, age, ethn, race, sex, vent, relat, region, arm, sentim_final, rasch_score_total, trust_score, stofhlascore)
    
all(family_info$uid==colnames(feature_info))
...
```{r distance}
  ...
  hc<-hclust(d1)
  plot(hc, labels=family_info$uid, cex=0.5) +
    abline(h=15)
  hclusters<-cutree(hc, h=15)
```

```{r explore_clusters}
      table(true=family_info$sentim_final,
              cluster=hclusters, useNA = "ifany")
```

```{r traits_by_cluster}
  family_info$cluster<-hclusters
  
  Table1<-CreateTableOne(data=family_info, strata = "cluster")
  Table1
